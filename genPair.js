...
var productionServer = process.env.NODE_WSS; //Private GETH MainNet Node
var delayTime = process.env.DELAY;
...
function generateApePanPairs() { //includes combo's of other stable pairs USDC's etc, no liquidity filtering, no weth only filtering
    ...
    for (let i = 0; i < apeMasterDB.length; i++) {
        if (pairExists(apeMasterDB[i].pairToken0, apeMasterDB[i].pairToken1)) {
            ...
            ABPairsWithIntersection.push({
                "ape": apeMasterDB[i],
                "pancake": panMasterDB[index],
                "order": "true"
            })
...
//queryPairsList('pancake'); //run first to get pairs
...
async function queryPairsList(dex) {
    console.log('Running QUERY PAIR LIST : ' + dex);
    var factoryContract;
    var filename;
    var pairLength
    if (dex == 'pancake') {
    ...
    for (let i = 0; i < pairLength; i++) { //set i to the last batch, batching to not go over nodejs GC limits for PCS
        console.log(i + '\n');
        if ((i+1) % process.env.BATCH_SIZE == 0){
            console.log('Batch Processed: Waiting for Next Batch');
            await delay(delayTime);
        }
        ...
        try {
            ...
            var toWrite = {
                ...
                "pairToken0": pairToken0,
                "pairToken1": pairToken1,
                "decimals0": decimal0,
                "decimals1": decimal1,
                "symbol0": symbol0,
                "symbol1": symbol1
    ...
    writePairList(pairMaster, filename)
    setTimeout(function () {
        process.exit()
    }, 5000)
